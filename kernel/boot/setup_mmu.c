...
__attribute__((section(".boot.bss"))) static int started;

__attribute__((aligned(4096))) __attribute__((section(".boot.bss"))) pte_t pkernel_pgtbl[VA_N_VPN - 2][1 << PN_WIDTH];
...
__attribute__((section(".boot.text"))) void setup_mmu(void)
{
    if (!started)
    {
        ...
        for (uint64 phy_pn = 0; phy_pn != (1ul << (PN_WIDTH - 1)); phy_pn++)
        {
        ...
        // no execute permission given in PMA;
        for (uint64 phy_pn = 0; phy_pn != (1ul << (PN_WIDTH - 2)); phy_pn++)
        {
            pte_t pte = (PTE_FLAG_V | PTE_FLAG_R | PTE_FLAG_W);
            uint64 p_addr = (phy_pn << ((VA_N_VPN - 1) * PN_WIDTH + PAGE_OFFSET_WIDTH));
            pte |= PA_PTE(p_addr);
            pkernel_pgtbl[VA_N_VPN - 3][phy_pn | (1ul << (PN_WIDTH - 1))] = pte;
        }

        // connecting high pagtables;
        for (uint64 i = VA_N_VPN - 3; i != 0; i--)
        {
            uint64 pn = (1 << PN_WIDTH) - 1;
